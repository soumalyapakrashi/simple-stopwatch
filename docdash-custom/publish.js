 ...
 * @param {array<object>} members.containers
...
function buildNav(members) {
    ...
    var defaultOrder = [
        'Components', 'Containers', 'Classes', 'Modules', 'Externals', 'Events', 'Namespaces', 'Mixins', 'Tutorials', 'Interfaces', 'Global'
    ];
    var order = docdash.sectionOrder || defaultOrder;
    var sections = {
        Components: buildMemberNav(members.components, 'Components', seen, linkto),
        Containers: buildMemberNav(members.containers, 'Containers', seen, linkto),
...
exports.publish = function (taffyData, opts, tutorials) {
    ...
    // Remove all undocumented doclets and other doclets like private doclets.
    // For more information, refer to 'jsdoc/lib/jsdoc/util/templateHelper.js~prune()'
    ...
    // Get all the member doclets (classes, modules, mixins, etc.) - The ones which gets
    // a h3 heading in the template.
    ...
    var containers = taffy(members.containers);
    ...
    Object.keys(helper.longnameToUrl).forEach(function (longname) {
        ...
        var myContainers = helper.find(containers, { longname: longname });
        if (myContainers.length) {
            generate('Container', myContainers[0].name, myContainers, helper.longnameToUrl[longname]);
        }
